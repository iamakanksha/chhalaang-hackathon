...
np.random.seed(42)
normal_data = np.random.normal(loc=0, scale=1, size=(100, 2))  # Normal data
anomaly_data = np.random.normal(loc=4, scale=1, size=(20, 2))  # Anomaly data
print(normal_data)
print(anomaly_data)
...
#df['encryptedHexCardNo']= df['encryptedHexCardNo'].convert_objects(convert_numeric=True)
 df = df.drop('encryptedHexCardNo', axis=1)

df.info()

df = df.drop('encryptedPan', axis=1)
...
normal_instances = df[predictions == 1]
